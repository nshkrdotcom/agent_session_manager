defmodule AgentSessionManager.Adapters.SQLiteSessionStoreTest do
  ...
  setup _ctx do
    db_path =
      Path.join(
        System.tmp_dir!(),
        "asm_sqlite_test_#{System.unique_integer([:positive, :monotonic])}.db"
      )
  ...
  describe "append_event_with_sequence/2" do
    ...
    test "sequences are per-session", %{store: store} do
      ...
      # Both should start at 1 since they're different sessions
      assert stored1.sequence_number == 1
  ...
  describe "get_events/3" do
    ...
    test "filters by run_id", %{store: store, session_id: sid} do
      # Add an event for a different run
